# Documentation

| Guide | For |
| --- | --- |
| [Using the app](using.md) | Operators: every surface, in order |
| [Destinations](destinations.md) | DELETE · STORE · EXPLORE · DECIDE · EXECUTE, and decision tiers |
| [Architecture](architecture.md) | Builders: where code lives and what owns which gate |
| [Setup](setup.md) | Public Railway fixture, local try, Notion, production |
| [Brand](../TRADEMARKS.md) | How to credit ApexForm Life |

The product decides what happens to a thought once it appears. Code owns destinations, tiers, lint, and maths. A model may fill interpretation and canvas prose. The human retains judgment.
